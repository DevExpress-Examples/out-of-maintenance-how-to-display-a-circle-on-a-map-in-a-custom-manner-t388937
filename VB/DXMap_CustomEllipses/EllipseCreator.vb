...
Namespace DXMap_CustomEllipses
    ...
    Public Class EquidistantCircleCreator
        ...
        Protected Overrides Function CreateCircleCore(ByVal center As GeoPoint, ByVal radius As Double) As MapPolyline
            Dim ellipse As MapPolyline = New MapPolyline()
            Dim [step] As Double = Math.PI / 180.0
            Dim i As Double = 0
            While i < 2.0 * Math.PI
                Dim distanceDelta As Point = New Point(radius * Math.Cos(i), radius * Math.Sin(i))
    ...
    Public Class ScreenCircleCreator
        ...
        Protected Overrides Function CreateCircleCore(ByVal center As GeoPoint, ByVal radius As Double) As MapPolyline
            ...
            Dim rightPoint As GeoPoint = New GeoPoint(center.Latitude, center.Longitude + delta.Width)
            ...
            Dim i As Double = 0
            While i < 2.0 * Math.PI
                Dim point As Point = New Point(centerPointOnScreen.X + screenRadius * Math.Cos(i), centerPointOnScreen.Y - screenRadius * Math.Sin(i))
